...
float getPitchIMU(){
  ...
  //float pitch_new = atan2(2.0*(q.y*q.z + q.w*q.x), q.w*q.w - q.x*q.x - q.y*q.y + q.z*q.z); //test
...
// read the yaw value from the IMU
float getYawIMU(){
  // static variable used for debouncing
  static float yaw;
  Quaternion q;           // [w, x, y, z]         quaternion container
  ...
  // calculate the angle of the robot
  float yaw_new = atan2(2.0*(q.x*q.y + q.w*q.z), q.w*q.w + q.x*q.x - q.y*q.y - q.z*q.z);

  // a bit of debouncing
  if (abs(yaw_new - yaw) > 0.1) yaw += _sign(yaw_new - yaw) * 0.01;
  else yaw = yaw_new;
...
float getRollIMU(){
  ...
  // calculate the angle of the robot
  float roll_new = asin(-2.0*(q.x*q.z - q.w*q.y));
